...
const CloseTicketForm: React.FC<CloseTicketFormProps> = ({
  onSubmit,
  isSubmitting = false,
}) => {
    ...
    >
      ...
      <div className="mt-4 flex justify-end">
        <button
          ...
          className="flex items-center gap-2 text-sm border px-4 py-2 text-white bg-red-600 rounded hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isSubmitting && (
            <svg
          ...
          {isSubmitting ? 'Sending...' : 'Close Ticket'}
        </button>
      </div>
    </form>
  );
