...
const EditSLAPolicyModal: React.FC<Props> = ({ isOpen, onClose, onUpdated, sla_policy }) => {
  const { editSLAPolicy} = useContext(SLAContext);
  ...
  useEffect(() => {
    if (isOpen && sla_policy) {
      setForm({
            name: sla_policy.name,
            response_time_min: sla_policy.response_time_min,
            resolve_time_min: sla_policy.resolve_time_min,
      });
    }
    if (!isOpen) {
      setError(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
     e.preventDefault();
    if (!sla_policy || !sla_policy.id) {
      console.error("No item provided for update");
      return;
    }
    setSubmitting(true);
    setError(null);
    ...
    try {
      ...
      const response = await editSLAPolicy(formData);
      if (response.success) {
...
export default EditSLAPolicyModal;
