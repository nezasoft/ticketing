...
type SLARuleListProps ={
    sla_rules: SLARule[];
    onUpdated: () => void;
};

const SLARuleList: React.FC<SLARuleListProps> = ({sla_rules, onUpdated}) => 
{
    const [currentPage, setCurrentPage] = useState(1);
    const [searchTerm, setSearchTerm] = useState("");
    const [selectedSLARule, setSelectedSLARule] = useState<SLARule | null>(null);
    ...
    const filteredSLARules = sla_rules.filter((sla_rule)=>
    {
        const matchesText = 
        String(sla_rule.policy ?? "").toLowerCase().includes(term) ||
        String(sla_rule.priority ?? "").toLowerCase().includes(term) ||
        String(sla_rule.channel ?? "").toLowerCase().includes(term) ||
        String(sla_rule.customer_type ?? "").toLowerCase().includes(term);
        return (
            matchesText
        )
    });
    const paginatedSLARules = filteredSLARules.slice((currentPage - 1) * itemsPerPage,currentPage * itemsPerPage);
    useEffect(()=>
    {
        setCurrentPage(1);
    },[sla_rules, searchTerm]);
    ...
    const handleEditClick = (sla_rule: SLARule) => 
    {
    ...
    const handleDelete = async (ruleId: number) => 
    {
        ...
        try{
            setOpenDropdown(null);
            const response = await slaCtx?.deleteSLARule?.(ruleId);
            if(response?.success)
            {
    ...
    const handleExportToExcel = () =>   
    {
        const worksheet = XLSX.utils.json_to_sheet(
            filteredSLARules.map((e)=>({
                Policy: e.policy,
                CustomerType: e.customer_type,
                Priority: e.priority,
                Channel: e.channel,
                ResponseTime: e.response_time,
                ResolveTime: e.resolve_time
            }))
        );
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet,"SLA Rules");
        const excelBuffer = XLSX.write(workbook,{bookType: "xlsx", type: "array"});
        const blob = new Blob([excelBuffer,{type:"application/octet-stream"}]);
        saveAs(blob,"sla_rules.xlsx");
    }

    const handleExportToPDF = () =>
    {
        const doc = new jsPDF();
        const tableColumn = ["Policy","Customer Type","Priority","Channel","Response Time","Resolve Time"];
        const tableRows =filteredSLARules.map((e)=> [
            e.policy || "",
            e.customer_type || "",
            e.priority || "",
            e.channel || "",
 ...
 useEffect(() => {
    ...
    const totalPages = Math.ceil(filteredSLARules.length / itemsPerPage);
    return (
    <div className="p-4">
      {/* Search + Export */}
      <div className="flex flex-wrap gap-2 items-center justify-between mb-4">
        <div className="flex">
          <div className="relative w-full md:w-64">
            ...
            <input
              type="text"
              placeholder="Search Policy, Priority, Channel"
              value={searchTerm}
      ...
      <div className="overflow-x-auto bg-white dark:bg-zinc-900 rounded-lg shadow">
        ...
        <tbody>
          {paginatedSLARules.length > 0 ? (
            paginatedSLARules.map((sla_rule, index) => (
              <tr
                key={sla_rule.id}
              ...
              >
                ...
                <td className="px-4 py-3 text-violet-600 font-semibold">{sla_rule.policy}</td>
                <td className="px-4 py-3 text-violet-600 font-semibold">{sla_rule.customer_type}</td>
                <td className="px-4 py-3 text-violet-600 font-semibold">{sla_rule.priority}</td>
                <td className="px-4 py-3 text-violet-600 font-semibold">{sla_rule.channel}</td>
                <td className="px-4 py-3 text-violet-600 font-semibold">{sla_rule.response_time}</td>
                <td className="px-4 py-3 text-violet-600 font-semibold">{sla_rule.resolve_time}</td>
                {/* Right-aligned action menu */}
                <td className="relative px-4 py-3 text-right">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setOpenDropdown(openDropdown === sla_rule.id ? null : sla_rule.id);
                      setSelectedSLARule(sla_rule); 
                    }}
                  ...
                  {openDropdown === sla_rule.id && (
                    <div  ref={dropdownRef} className="absolute right-0 mt-2 w-28 bg-white dark:bg-zinc-800 shadow-lg rounded-md text-sm border dark:border-zinc-700 z-50">
                      <button
                        onClick={() => handleEditClick(sla_rule)}
                        className="block w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-zinc-700 text-left"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => handleDelete(sla_rule.id)}
